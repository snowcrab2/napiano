...
<link rel="stylesheet" href="/resources/css/community/get.css">
	...
	<section>
		<div class="sectionHeder">
			...
			<div class="boardWriter">
				<strong><c:out value='${cboard.writer }'/></strong>
			</div>
			
			<!-- 작성자가 같을때(아이디) 수정,삭제 노출 -->
			<c:if test="${loginMember.id == cboard.id}">
			...
			<div class="boardTimeHit">
				<span><fmt:formatDate pattern="MM/dd" value="${cboard.updateDate }"/></span>
				<span class="divider">·</span>
				<span>조회수 <c:out value="${cboard.hit }"/></span>
			</div>
		</div>
		<div class="boardContent">
			<h3><c:out value='${cboard.title }'/></h3>
			<c:out value="${cboard.content }"/>
		</div>
